...
function renderQuestion(q, index) {
  const lang = "zh"; // ✅ Chỉ hỗ trợ tiếng Trung
  if (q.language !== "zh") {
    alert("⚠️ Chỉ hỗ trợ luyện dịch tiếng Trung.");
    window.location.href = "select-quiz.html";
    return;
  }
...
`;
  ...
  helpBtn.onclick = () => {
    ...
    const updated = accumulatedMatched.map((c) => c || "＿").join("");
  ...
  nextBtn.onclick = () => {
    ...
    if (currentIndex < questions.length) {
      ...
      let content = `<h2>🎉 Bạn đã hoàn thành bài luyện dịch tiếng Trung!</h2>`;
